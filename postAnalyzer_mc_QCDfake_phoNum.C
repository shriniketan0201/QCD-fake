...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++){
    ...
    phoCandNum   = getPhoCandNum(225.0,1.4442);
    phoCandNum_QCD = getPhoCandNum_QCDsideband(225.0,1.4442);
    phoCandNum_QCDsbUp = getPhoCandNum_QCDsbUp(225.0,1.4442);
    phoCandNum_QCDsbDown = getPhoCandNum_QCDsbDown(225.0,1.4442);
    ...
    for(int p = 0; p < genPhotons.size(); p++)    {
      float dRtemp = 9999.9;
      if(phoCandNum.size() > 0)dRtemp = dR(mcEta->at(genPhotons[p]),mcPhi->at(genPhotons[p]),phoSCEta->at(phoCandNum[0]),phoSCPhi->at(phoCandNum[0]));
      if(dRtemp < dRmin)dRmin = dRtemp;
      dRtemp = 9999.9;
      if(phoCandNum_QCD.size() > 0)dRtemp = dR(mcEta->at(genPhotons[p]),mcPhi->at(genPhotons[p]),phoSCEta->at(phoCandNum_QCD[0]),phoSCPhi->at(phoCandNum_QCD[0]));
      if(dRtemp < dRmin_QCD)dRmin_QCD = dRtemp;
      dRtemp = 9999.9;
      if(phoCandNum_QCDsbUp.size() > 0)dRtemp = dR(mcEta->at(genPhotons[p]),mcPhi->at(genPhotons[p]),phoSCEta->at(phoCandNum_QCDsbUp[0]),phoSCPhi->at(phoCandNum_QCDsbUp[0]));
      if(dRtemp < dRmin_QCDsbUp)dRmin_QCDsbUp = dRtemp;
      dRtemp = 9999.9;
      if(phoCandNum_QCDsbDown.size() > 0)dRtemp = dR(mcEta->at(genPhotons[p]),mcPhi->at(genPhotons[p]),phoSCEta->at(phoCandNum_QCDsbDown[0]),phoSCPhi->at(phoCandNum_QCDsbDown[0]));
      if(dRtemp < dRmin_QCDsbDown)dRmin_QCDsbDown = dRtemp;
    }
...
void postAnalyzer::fillHistosNum(int sysNumber, int index,double event_weight)
{
  ...
  if(uncorrectedPhoEt > 225.0 && uncorrectedPhoEt <= 250.0)
    histoNumber = 1;
  else if(uncorrectedPhoEt > 250.0 && uncorrectedPhoEt <= 300.0)
    histoNumber = 2;
  else if(uncorrectedPhoEt > 300.0 && uncorrectedPhoEt <= 400.0)
    histoNumber = 3;
  else if(uncorrectedPhoEt > 400.0 && uncorrectedPhoEt <= 600.0)
    histoNumber = 4;
  else if(uncorrectedPhoEt > 600.0)
    histoNumber = 5;
  if(histoNumber > 0)
  {
    h_sieie_num[histoNumber][sysNumber]->Fill(phoSigmaIEtaIEtaFull5x5->at(index),event_weight);
...
std::vector<int> postAnalyzer::getPhoCandNum_QCDsideband(double phoPtCut, double phoEtaCut){
  ...
  for(int p=0;p<nPho;p++)
  {
    ...
    bool photonId = (
         ((*phoHoverE)[p]                <  0.0230   ) &&
         ((*phohasPixelSeed)[p]              ==  0      ) &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) > 8.0 )  &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) < 14.0 )  &&
	 ( TMath::Max( ( (*phoPFNeuIso)[p] - rho*EAneutral((*phoSCEta)[p]) ), 0.0) < (7.25 + (9.73714e-03 * uncorrectedPhoEt) + (1.01605e-05 * pow(uncorrectedPhoEt, 2.0))) )  &&
         ( TMath::Max( ( (*phoPFPhoIso)[p] - rho*EAphoton((*phoSCEta)[p])  ), 0.0) < (0.17 + (2.99336e-03 * uncorrectedPhoEt)) )
    );
...
std::vector<int> postAnalyzer::getPhoCandNum_QCDsbUp(double phoPtCut, double phoEtaCut){
  ...
  for(int p=0;p<nPho;p++)
  {
    ...
    bool photonId = (
         ...
         ((*phohasPixelSeed)[p]              ==  0      ) &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) > 8.0 )  &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) < 16.0 )  &&
	 ( TMath::Max( ( (*phoPFNeuIso)[p] - rho*EAneutral((*phoSCEta)[p]) ), 0.0) < (7.25 + (9.73714e-03 * uncorrectedPhoEt) + (1.01605e-05 * pow(uncorrectedPhoEt, 2.0))) )  &&
         ( TMath::Max( ( (*phoPFPhoIso)[p] - rho*EAphoton((*phoSCEta)[p])  ), 0.0) < (0.17 + (2.99336e-03 * uncorrectedPhoEt)) )
    );
...
std::vector<int> postAnalyzer::getPhoCandNum_QCDsbDown(double phoPtCut, double phoEtaCut){
  ...
  for(int p=0;p<nPho;p++)
  {
    ...
    bool photonId = (
         ...
         ((*phohasPixelSeed)[p]              ==  0      ) &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) > 8.0 )  &&
         ( TMath::Max( ( (*phoPFChWorstIso)[p]  - rho*EAchargedworst((*phoSCEta)[p]) ), 0.0) < 12.0 )  &&
	 ( TMath::Max( ( (*phoPFNeuIso)[p] - rho*EAneutral((*phoSCEta)[p]) ), 0.0) < (7.25 + (9.73714e-03 * uncorrectedPhoEt) + (1.01605e-05 * pow(uncorrectedPhoEt, 2.0))) )  &&
         ( TMath::Max( ( (*phoPFPhoIso)[p] - rho*EAphoton((*phoSCEta)[p])  ), 0.0) < (0.17 + (2.99336e-03 * uncorrectedPhoEt)) )
       );
    bool noncoll = fabs((*phoSeedTime)[p]) < 3. && (*phoMIPTotEnergy)[p] < 4.9 && (*phoSigmaIEtaIEtaFull5x5)[p] > 0.001 && (*phoSigmaIPhiIPhiFull5x5)[p] > 0.001;
...
double postAnalyzer::sieie_correction(Float_t sieie, int shift){
  ...
  //  if(binNumber < 0 || binNumber > 14)    correction = 1.0;
  //  else if(shift == 0)    correction = central[binNumber];
  //  else if(shift == 1)    correction = up[binNumber];
  //  else if(shift == 2)    correction = down[binNumber];
